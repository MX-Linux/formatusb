...
print_header() {
    echo -e "${BLUE}========================================${NC}"
    echo -e "${BLUE}  Formatusb Release Script${NC}"
    echo -e "${BLUE}========================================${NC}"
    echo
}
...
# Validate version format (semantic versioning, YY.MM, or with alphanumeric suffixes)
validate_version() {
    local version=$1
    ...
    # Check semantic version format (major.minor.patch) or YY.MM format, allowing alphanumeric suffixes
    if ! [[ $clean_version =~ ^[0-9]+\.[0-9]+[a-zA-Z0-9]*(\.[0-9]+[a-zA-Z0-9]*)?$ ]]; then
        print_error "Invalid version format: $version"
        echo "Expected formats: 1.0.0, v1.0.0, YY.MM (like 25.06), or YY.MMsuffix (like 26.01arch)"
        exit 1
    fi
...
get_latest_tag() {
    {
        ...
        if [[ $clean_tag =~ ^[0-9]+\.[0-9]+[a-zA-Z0-9]*(\.[0-9]+[a-zA-Z0-9]*)?$ ]]; then
            printf '%s\n' "$tag"
        fi
    done | sort -V | tail -n1
}
...
update_aur_package() {
    ...
    local tarball_url="https://github.com/MX-Linux/formatusb/archive/refs/tags/${version}.tar.gz"
